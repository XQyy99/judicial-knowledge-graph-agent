...
bert_path_or_name=r"D:\dl\bert-crf-token_classification_ner-master\pretrain\bert-base-chinese"
num_labels=7
class CRF(nn.Module):
...
class BertCRFModel(nn.Module):
    ...
    def forward(
        ...
        loss = None
        if labels is not None:
            # CRF 计算 loss（注意 mask 必须是 byte 类型）
            loss = -1 * self.crf(emissions=logits, tags=labels, mask=attention_mask.byte())

        # Step 4: 返回 Hugging Face 的 TokenClassifierOutput 格式
        if not return_dict:
